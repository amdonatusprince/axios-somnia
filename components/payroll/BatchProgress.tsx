...
interface BatchProgressProps {
  ...
  txHash?: string
  error?: string
  /** Shown on success: per-employee payout lines (human USD amounts). */
  lineItems?: Array<{ id: string; name: string; amountUsd: number }>
}
        ...
        {status === 'success' && (
          ...
          >
            ...
            {lineItems && lineItems.length > 0 && (
              <ul className="mt-3 space-y-1.5 text-left text-sm border-t border-[var(--border-default)] pt-3">
                {lineItems.map((row) => (
                  <li
                    key={row.id}
                  ...
                  >
                    ...
                    <span className="font-mono shrink-0 text-[var(--text-primary)]">
