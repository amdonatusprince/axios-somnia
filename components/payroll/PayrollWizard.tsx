...
export function PayrollWizard({ employees, employerId, onComplete }: PayrollWizardProps) {
  ...
  async function executePayroll() {
    ...
    try {
      ...
      if (!calldataRes.ok) {
        const data = (await calldataRes.json().catch(() => ({}))) as {
          error?: string
          detail?: string
          code?: string
          availableUsd?: number
        ...
        if (data.availableUsd != null && data.requiredUsd != null) {
          parts.push(
            `Available in PayrollTreasury: $${data.availableUsd.toFixed(2)} · Required for this run: $${data.requiredUsd.toFixed(2)}`,
          )
        }
        throw new Error(parts.join(' ') || 'Failed to prepare payroll')
      }
          ...
          {step === 3 && (
            <BatchProgress
              ...
              lineItems={items.map((i) => ({
                id: i.employee.id,
                name: `${i.employee.first_name ?? ''} ${i.employee.last_name ?? ''}`.trim(),
                amountUsd: i.amount,
              }))}
            />
          )}
