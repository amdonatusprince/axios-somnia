...
import { bridgeRequest, isBridgeMock } from '@/lib/bridge'
...
function getAppUrl(appUrlOverride?: string) {
  return (appUrlOverride ?? process.env.NEXT_PUBLIC_APP_URL ?? 'https://axios.xyz').replace(/\/$/, '')
}
...
export async function ensureEmployeeKycLink(
  ...
  if (!process.env.BRIDGE_API_KEY && !isBridgeMock()) {
    return null
  }
...
export async function sendEmployeeInviteEmail(opts: {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Axios <donatusprince@gmail.com>',
        to: opts.to,
        subject: `${opts.companyName} invited you to Axios`,
        html: [
          `<p>Hi ${firstName},</p>`,
          `<p>${opts.companyName} uses Axios to pay their team. Click the link below to set up your account and receive your salary directly to your digital wallet.</p>`,
