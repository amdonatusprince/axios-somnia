...
///         Enforces ComplianceRegistry checks on registration when a policy is set.
contract EmployeeRegistry {
    ...
    function registerEmployee(
        ...
        if (cfg.policyId > 0) {
            bool authorized = complianceRegistry.isAuthorized(cfg.policyId, wallet);
    ...
    function getWallet(bytes32 employeeIdHash) external view returns (address) {
        address[] memory wallets = employerWallets[bytes32(0)];
