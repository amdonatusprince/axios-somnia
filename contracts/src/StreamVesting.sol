...
contract StreamVesting {
    ...
    event StreamReleased(uint256 indexed streamId, address indexed employee, uint256 amount, bytes32 payrollMemo);
    ...
    function createStream(
        ...
        require(payToken.transferFrom(msg.sender, address(this), totalAmount), "transferFrom failed");
    ...
    function release(uint256 streamId) external {
        ...
        require(payToken.transfer(s.employee, releasable), "transfer failed");

        emit StreamReleased(streamId, s.employee, releasable, s.payrollMemo);
    }
    ...
    function claimAccrued(address employee) external returns (bytes32 txHash) {
        ...
        require(payToken.transfer(employee, totalReleasable), "transfer failed");
    ...
    function cancelStream(uint256 streamId) external {
        ...
        if (remaining > 0) {
            require(payToken.transfer(s.employer, remaining), "transfer failed");
        }
