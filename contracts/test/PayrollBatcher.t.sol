...
contract PayrollBatcherTest is Test {
    ...
    function testEmployerWithdrawsAvailableBalance() public {
        bytes32 employerId = keccak256(abi.encodePacked(employer));
        uint256 beforeBal = token.balanceOf(employer);

        vm.prank(employer);
        treasury.withdraw(200_000e6);

        assertEq(token.balanceOf(employer), beforeBal + 200_000e6);
        assertEq(treasury.getAvailableBalance(employerId), 800_000e6);
    }
}
