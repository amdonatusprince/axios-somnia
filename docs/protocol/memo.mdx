...
description: "32-byte payroll memos and compliance checks on Somnia."
---

On **Somnia testnet**, Axios attaches **32-byte memos** to **sUSDC** ERC-20 transfers so payroll metadata is auditable on-chain. The same structural ideas apply as on other stacks that used TIP-20–style memo fields.

Compliance uses the deployed **`ComplianceRegistry`** contract (allow/block policies) rather than a single hardcoded precompile — see [Employee Registry](/docs/on-chain/employee-registry) and the protocol contracts table.

This architecture keeps payment details machine-readable and ties screening to on-chain policy IDs.
...
Global enterprise payroll must respect strict jurisdictional boundaries. Consequently, every Axios transfer is automatically screened by the `TIP403Registry` precompile on execution.
