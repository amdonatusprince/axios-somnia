...
description: "Non-custodial TEE-backed transaction signing for the Axios autonomous agent."
---

Axios's autonomous payroll agent signs Tempo L1 transactions using [Lit Protocol Chipotle](https://developer.litprotocol.com) — a distributed Trusted Execution Environment network. The agent wallet is a PKP (Programmable Key Pair) whose private key is generated and held inside Lit's TEE network. It never exists in one place, and no single node can sign independently.

The integration uses the Chipotle REST API directly. No SDK node client is required.

## Why Axios Uses Lit

Payroll is the highest-stakes transaction an employer can authorize. Axios uses Lit Protocol for three reasons that are specific to the payroll context and cannot be replicated with a hot wallet or a traditional HSM:

**1. Employer-delegated, non-custodial agent authorization**

Axios operates as an autonomous payroll agent on behalf of employers. The employer should not need to hand Axios their private key to run payroll. With Lit, the employer's PKP wallet signs transactions — Axios holds a delegatee key that can trigger signing but has no ability to extract the private key or sign anything outside the authorized policy. The employer retains cryptographic ownership of the signing identity at all times.

**2. Programmable spend policies enforced in the TEE**

The Contract Whitelist Policy restricts the PKP to only sign transactions targeting `PayrollBatcher` and `YieldRouter` on Tempo L1. No other contract can be called by the agent — not because Axios's server enforces it, but because the Lit Action code running inside the TEE enforces it before signing. This turns compliance guardrails from an application-layer promise into a cryptographic guarantee.

**3. Verifiable on-chain audit trail without a centralized key custodian**

Every payroll disbursement signed by the PKP wallet produces a Tempo L1 transaction with a `from` address that is cryptographically linked to a Lit-managed key. An auditor can verify, without trusting Axios's servers, that the signing key was controlled by the Lit TEE network — not by a hot wallet sitting on Axios's infrastructure. For ISO 20022-compliant payroll, this separation between authorization (employer PKP) and execution (Axios agent) is the moat.

## Signing Architecture

```mermaid
flowchart LR
    A[Axios Agent] -->|unsigned EIP-1559 tx| B(lib/vincent-agent.ts)
...
**Groups** control which Lit Actions can execute and which PKPs they can access. The Axios signing action is pre-authorized in a group that includes the Axios PKP wallet.
...
The Axios agent PKP wallet on the dev network is `0x3324a8B644a78ed5c9EEBbD0e661b67FE417342F`. Fund this address on Tempo L1 (chainId `4217`) with pathUSD for payroll operations. The PKP pays its own gas.

## Contract Whitelist

For production, restrict the PKP to only sign transactions targeting approved Axios contracts by adding policy enforcement to the Lit Action:
