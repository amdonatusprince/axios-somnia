...
The `EmployeeRegistry` contract links employer IDs to **employee wallet addresses** on **Mezo**, with optional checks against the **`ComplianceRegistry`** contract (policy IDs per employer).
...
The registry stores:
- `wallet`: The **Mezo** `0x…` receiving address.
