...
Axios's autonomous payroll agent signs Tempo L1 transactions using Lit Protocol Chipotle (v3). The agent's private key is held inside Lit's distributed TEE network — it never exists in one place, and no single node can sign independently.
...
3. **Signing group** — `axios-payroll-signers` (group id 1) with PKP added
