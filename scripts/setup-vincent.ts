 ...
 *
 * Creates a complete Lit Protocol Chipotle signing setup for the Axios agent:
...
async function main() {
  ...
  const axiosGroup = existingGroups.find((g) => g.name === 'axios-payroll-signers')
  ...
  if (!axiosGroup) {
    ...
    await litApi(
      '/add_group',
      {
        method: 'POST',
        body: JSON.stringify({
          group_name: 'axios-payroll-signers',
          group_description: 'Axios autonomous payroll agent signing group',
    ...
    const created = groups.find((g) => g.name === 'axios-payroll-signers')
    groupId = created ? parseInt(created.id, 16) : 1
    console.log('Group created, id:', groupId)
  } else {
    groupId = parseInt(axiosGroup.id, 16)
  ...
  if (!usageKey) {
    ...
    const usage = (await litApi(
      '/add_usage_api_key',
      {
        method: 'POST',
        body: JSON.stringify({
          name: 'axios-agent-execute',
          description: 'Execution key for Axios payroll agent signing',
