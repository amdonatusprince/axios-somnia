...
const PUBLIC_PREFIXES = [
  ...
  '/bridge-mock', // simulated Bridge KYC (BRIDGE_MOCK)
  '/api/bridge-mock/', // completes mock KYC in Supabase
  '/kyc/',
  '/legal/',
  '/_next/',
  ...
  '/api/auth/', // Wallet SIWE nonce + verify
...
// ─── Session decode (Axios JWT + legacy Privy cookie) ───────────────────────

/**
 * Decode legacy Privy JWT payload without signature verification (cookie only).
 */
...
async function decodeSessionFromRequest(request: NextRequest): Promise<{ sub: string } | null> {
  const axiosCookie = request.cookies.get('axios-token')?.value
  if (axiosCookie) {
    const v = await verifySessionJwt(axiosCookie)
    if (v) return v
  }
  const privyToken =
    request.cookies.get('privy-token')?.value ?? request.cookies.get('privy_token')?.value
  if (privyToken) return decodePrivyToken(privyToken)
  return null
}
...
export async function middleware(request: NextRequest) {
  ...
  if (isPublic(pathname)) {
    ...
    if (isAuthRoute(pathname)) {
      const decoded = await decodeSessionFromRequest(request)
      if (decoded) {
