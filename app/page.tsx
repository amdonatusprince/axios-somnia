            ...
            <div className="flex flex-col gap-4 text-left">
              <div className="rounded-2xl border border-white/5 bg-[#101A33] p-5">
                ...
                <div className="mt-4 flex items-center gap-2 text-xs text-accent">
                  ...
                  Compliance registry clear · ISO 20022 memo encoded
                </div>
              </div>
      ...
      <div className="mx-auto max-w-5xl px-6">
        <FadeInUp>
          ...
          <p className="mb-16 max-w-2xl text-lg text-white/50">
            ...
            and loses more to hidden FX spread. Axios settles the same flow in seconds with audit-ready payment data attached.
          </p>
        </FadeInUp>
    ...
    <section id="features" className="py-32">
      <div className="mx-auto max-w-5xl px-6">
        <FadeInUp>
          ...
          <p className="mb-16 max-w-2xl text-lg text-white/50">
            Axios replaces multi-day payroll settlement with 0.4-second batch execution, embedded wallets, protocol-level compliance, gas sponsorship, and yield on treasury float.
          </p>
        </FadeInUp>
...
const COMPARISON_ROWS = [
  { feature: 'Settlement speed', axios: 'Under 1 second', deel: '1–5 days', bitwage: '1–3 days', wire: '1–5 days' },
  { feature: 'Transaction cost', axios: '< $0.01', deel: '$5–$25', bitwage: '1% fee', wire: '$25–$75' },
  { feature: 'FX fee', axios: '0%', deel: '0.5–2%', bitwage: '0.5%', wire: '1–3%' },
  { feature: 'Visa cards for employees', axios: true, deel: false, bitwage: false, wire: false },
  { feature: 'Yield on treasury', axios: '3.5%+ APY', deel: false, bitwage: false, wire: false },
  { feature: 'Compliance built-in', axios: 'On-chain', deel: 'Manual', bitwage: 'Manual', wire: 'Manual' },
  { feature: 'Coverage', axios: '120+ countries', deel: '150+', bitwage: '100+', wire: '200+' },
  { feature: 'No crypto knowledge needed', axios: true, deel: true, bitwage: false, wire: true },
]
              ...
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="border-x border-accent/10 bg-accent/5 px-6 py-4 text-sm font-semibold">
                    <span className="text-accent">Axios</span>
                  </th>
                ...
                {COMPARISON_ROWS.map((row, index) => (
                  <tr key={row.feature} className={`border-b border-white/5 ${index % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>
                    ...
                    <td className="border-x border-accent/10 bg-accent/5 px-6 py-3.5 text-center">
                      {typeof row.axios === 'boolean' ? (
                        row.axios ? <CheckCircle className="mx-auto h-4 w-4 text-accent" /> : <XCircle className="mx-auto h-4 w-4 text-white/20" />
                      ) : (
                        <span className="text-sm font-semibold text-accent">{row.axios}</span>
                      )}
                    </td>
...
const FAQS = [
  ...
  },
  {
    ...
    a: 'Axios exposes machine-payable payroll APIs behind HTTP 402 (x402): agents pay per call to probe compliance, treasury readiness, and execution — no shared long-lived API keys with unlimited blast radius.',
  },
  {
  ...
  },
  {
    ...
    a: 'Axios handles the payment rails. You remain responsible for labor law compliance in each jurisdiction. We recommend structuring stablecoin payroll as an optional settlement preference for employees.',
  },
]
