...
function getComplianceRegistryStatus(data: EmployerTeamDetailResponse['complianceEvents']) {
  const latestCheck = data.find((event) => event.event_type === 'mpp_check')
  if (!latestCheck) return 'pending'
  return latestCheck.result === 'BLOCKED' ? 'blocked' : 'authorized'
}
          ...
          {hasCard ? (
            ...
            <div className="rounded-2xl border border-dashed border-[var(--border-default)] bg-[var(--bg-base)] p-5">
              <p className="text-sm text-[var(--text-secondary)]">
                No Axios card has been issued for this employee yet.
              </p>
              {cardReadyToIssue ? (
...
function ComplianceTab({
  ...
  const complianceRegistryStatus = getComplianceRegistryStatus(complianceEvents)
        ...
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5">
          ...
          <div className="mt-4">
            <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium ${
              complianceRegistryStatus === 'authorized'
                ? 'bg-[var(--accent-subtle)] text-[var(--accent)]'
                : complianceRegistryStatus === 'blocked'
