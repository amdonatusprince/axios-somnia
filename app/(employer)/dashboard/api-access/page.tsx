  ...
  return (
    <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 sm:p-6">
      ...
      <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
        Generate a one-time key for your treasury or payroll agents. Axios stores only a hash after creation, so the raw key is only visible at generation time.
      </p>
      ...
      {generatedKey ? (
        <div className="mt-5 space-y-3">
          ...
          <p className="text-xs text-[var(--status-pending)]">Copy this key now. Axios will not show it again after this page refreshes.</p>
        </div>
      ) : null}
...
export default function ApiAccessPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-[var(--text-primary)]">Recent x402 activity</h2>
        {receiptsLoading ? (
          <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-8 text-center text-sm text-[var(--text-muted)]">
            Loading receipts…
          </div>
          ...
          <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-8 text-center text-sm text-[var(--text-muted)]">
            Payment receipts will appear here after agents or operators complete x402-paid API calls.
          </div>
        )}
      </div>
