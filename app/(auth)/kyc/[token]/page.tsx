...
async function getKycState(token: string, appUrlOverride?: string) {
  ...
  if (employee.kyc_status === 'approved') {
    return {
      ...
      description: `${displayName} is already cleared for Axios payroll.`,
      companyName,
    }
  }

  try {
    const link = await ensureEmployeeKycLink(employee, appUrlOverride)
    if (!link?.kycUrl) {
      return {
...
export default async function KycVerificationPage({ params }: PageProps) {
  ...
  const data = await getKycState(token, appUrl)
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <ul className="mt-3 space-y-3 text-sm leading-6 text-[var(--text-secondary)]">
                      ...
                      <li>Axios writes the updated status back into your payroll profile after the Bridge flow completes.</li>
                    </ul>
                  </div>
