...
export default function RegisterPage() {
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      const res = await authedFetch('/api/employers')
      if (cancelled) return
      if (res.ok) {
        const data = (await res.json()) as { employer?: { id: string } | null }
        if (data.employer?.id) {
    ...
    try {
      const res = await authedFetch('/api/employers', {
        ...
        body: JSON.stringify({
          ...
          employerAdminWallet: address ?? undefined,
        }),
      })
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] flex">
      <div className="hidden lg:flex lg:w-[52%] flex-col justify-between p-12 bg-[var(--bg-surface)] border-r border-[var(--border-default)] relative overflow-hidden">
        ...
        >
          <AxiosLogo className="gap-2.5" markClassName="h-8 w-8" labelClassName="text-[var(--text-primary)] text-lg tracking-tight" />
        </motion.div>
        ...
        <div className="relative z-10">
          ...
          >
            <p className="text-xs font-mono text-[var(--accent)] tracking-widest uppercase mb-4">Get started in minutes</p>
            <h1 className="text-[2.5rem] leading-[1.1] font-bold text-[var(--text-primary)] tracking-tight mb-6">
              Run payroll <span className="text-[var(--accent)]">onchain</span> from day one.
            </h1>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        ...
        >
          <AxiosLogo className="lg:hidden mb-10" markClassName="h-7 w-7" labelClassName="text-[var(--text-primary)] text-base" />

          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">Set up your company</h2>
            <p className="mt-1.5 text-sm text-[var(--text-secondary)]">Tell us a bit about your organization</p>
          </div>
