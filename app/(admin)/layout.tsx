...
function AdminLayoutShell({ children }: { children: React.ReactNode }) {
  ...
  const adminLabel = address ? `${address.slice(0, 6)}…${address.slice(-4)}` : 'Platform admin'
    ...
    <div className="flex min-h-screen bg-[var(--bg-base)]">
      <aside className="hidden w-64 shrink-0 border-r border-[var(--border-default)] bg-[var(--bg-surface)] lg:flex lg:flex-col">
        <div className="border-b border-[var(--border-default)] px-5 py-4">
          <AxiosLogo markClassName="h-7 w-7" labelClassName="text-[var(--text-primary)] text-sm" />
        ...
        <div className="border-t border-[var(--border-default)] p-4">
          <p className="text-xs font-mono text-[var(--text-muted)]">{adminLabel}</p>
          <button
