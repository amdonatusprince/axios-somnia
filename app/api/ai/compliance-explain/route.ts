...
export async function POST(req: NextRequest) {
  ...
  const result = await runClaudeJson<{
    ...
    system: [
      'You explain Axios compliance events to payroll operators in plain English.',
    ...
    fallback: () => ({
      explanation: body.description
        ? `Axios recorded a ${body.eventType ?? 'compliance'} event with result ${body.result ?? 'unknown'}: ${body.description}`
        : `Axios recorded a ${body.eventType ?? 'compliance'} event with result ${body.result ?? 'unknown'}.`,
