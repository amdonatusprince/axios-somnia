 ...
 * Requires session Bearer token or axios-token cookie. Claims the invite for the authenticated user.
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  if (existingEmployer) {
    return NextResponse.json(
      { error: 'This account is already registered as an employer. Use a separate wallet session to accept employee invites.' },
      { status: 403 },
    )
  }
