      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[var(--text-muted)] mt-0.5">
            From {payment.employer_company_name ?? 'employer'} treasury · {formatDate(payment.created_at)}
          </p>
        </div>
        <ChevronDown
          ...
          >
            <div className="p-5 space-y-4">
              <p className="text-xs text-[var(--text-secondary)]">
                Payroll from {payment.employer_company_name ?? 'employer'} treasury to your payroll wallet.
              </p>
              {/* Status chip */}
...
export default function PaymentsPage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    return payments.filter((p) => {
      ...
      const company = (p.employer_company_name ?? '').toLowerCase()
      return (
        label.includes(q) ||
        amount.includes(q) ||
        company.includes(q) ||
