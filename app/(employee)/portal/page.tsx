...
import {
  ...
  useEmployeeBalance,
  useEmployeePayments,
  useEmployerForEmployee,
  useEmployerTreasurySnapshot,
...
import { annualSalaryUsd, usdPerSecondFromAnnual } from '@/lib/salary'
...
export default function PortalHomePage() {
  ...
  const { data: employerTreasury } = useEmployerTreasurySnapshot(employee?.id)
  ...
  const walletMusd = balanceData?.wallet_musd_usd ?? balanceData?.available_usd ?? 0

  const annualForStream = annualSalaryUsd(employee?.salary_amount ?? null, employee?.pay_frequency)
  const salaryPerSecond =
    isStreaming && annualForStream != null ? usdPerSecondFromAnnual(annualForStream) : 0
  ...
  const quickActions = [
    { label: 'View Payments', href: '/portal/payments', icon: CreditCard, description: 'Full payment history' },
    {
      ...
      icon: CreditCard,
      description: employee?.bridge_card_id ? 'Card status and transfers' : 'Issue your Axios card',
    },
  ...
  return (
    <div className="max-w-[640px] mx-auto px-4 pt-8 pb-24 space-y-5">
      ...
      >
        {employerTreasury != null && (
          <div className="pb-4 border-b border-[var(--border-default)]">
            <p className="text-xs text-[var(--text-muted)] mb-1">Company treasury (available)</p>
            <p className="number-xl text-2xl text-[var(--text-primary)]">
              {formatUsd(employerTreasury.available_usd)}
            </p>
            <p className="text-xs text-[var(--text-muted)] mt-1">
              MUSD in PayrollTreasury for {employerTreasury.company_name ?? companyName} — pool used to fund payroll runs (same as the employer dashboard treasury view).
            </p>
          </div>
        )}
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-3 min-w-0">
            <div>
              <p className="text-xs text-[var(--text-muted)] mb-1">Payroll received</p>
              <p className="number-xl text-2xl text-[var(--text-primary)]">{formatUsd(payrollReceived)}</p>
              <p className="text-xs text-[var(--text-muted)] mt-1">
                Total credited from {companyName}&apos;s treasury via Axios payroll.
              </p>
            </div>
            <div className="pt-1 border-t border-[var(--border-default)]">
              <p className="text-xs text-[var(--text-muted)] mb-1">MUSD in payroll wallet</p>
              {isStreaming ? (
                <BalanceTicker
                  balance={walletMusd}
          ...
          {isStreaming && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--accent-subtle)] text-[var(--accent)] text-xs font-medium shrink-0">
        ...
        {lastPayment ? (
          <div className="pt-4 border-t border-[var(--border-default)]">
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              From {lastPayment.employer_company_name ?? companyName} treasury
            </p>
          </div>
        ) : (
          <div className="pt-4 border-t border-[var(--border-default)]">
