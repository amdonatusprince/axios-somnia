...
export default function CardActivatePage() {
  ...
  return (
    <div className="mx-auto max-w-[640px] space-y-6 px-4 pb-24 pt-6">
      <SectionHeader
        title="Activate Card"
        description="Issue your Axios payroll card once Bridge KYC is approved and your employee profile is linked."
      />

      <VisaCardDisplay
        last4={cardData?.card?.last4 ?? undefined}
        holderName={[employee?.first_name, employee?.last_name].filter(Boolean).join(' ') || employee?.email || 'Axios User'}
