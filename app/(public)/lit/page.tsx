...
export const metadata: Metadata = {
  title: 'Non-Custodial Payroll Signing | Axios × Lit Protocol',
  description:
    'How Axios uses Lit Protocol TEE-backed PKP wallets to sign payroll transactions — without a hot wallet, without a key custodian.',
}
...
const WHY_CARDS = [
  {
    ...
    body: 'Axios runs payroll on behalf of employers. With Lit, the employer\'s PKP wallet is the signing identity. Axios holds a delegatee key that can trigger signing but cannot extract the private key or sign anything outside the authorized policy. The employer retains cryptographic ownership at all times.',
  },
  {
    icon: Lock,
    title: 'Spend policies enforced inside the TEE',
    body: 'The signing Lit Action restricts the PKP to only sign transactions targeting PayrollBatcher and YieldRouter on Tempo L1. This is not an application-layer promise — it\'s code running inside a Verified TEE that the enclave executes before returning a signature. The policy cannot be bypassed by Axios\'s server.',
  },
  {
    icon: FileCheck,
    title: 'Verifiable on-chain audit trail',
    body: 'Every payroll disbursement produces a Tempo L1 transaction with a from address cryptographically linked to the Lit-managed PKP. An auditor can verify — without trusting Axios\'s servers — that the signing key was controlled by the Lit TEE network, not a hot wallet on Axios\'s infrastructure.',
  },
]
    ...
    <div className="space-y-20">
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <p className="text-lg text-[var(--text-secondary)] leading-relaxed">
          Axios&apos;s autonomous payroll agent signs Tempo L1 transactions using a PKP wallet
          ...
          Axios&apos;s server never holds the key.
        </p>
      </div>

      {/* Why section */}
      <div className="space-y-6">
        <h2 className="text-xl font-bold text-[var(--text-primary)]">Why Axios uses Lit</h2>
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-6 md:p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-center">
            {([
              { label: 'Axios Agent',     sub: 'unsigned EIP-1559 tx',       dim: false },
